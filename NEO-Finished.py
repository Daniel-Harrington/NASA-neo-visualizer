...
if display_choice.upper() == "Y":
    ...
    if display_choice.upper() == "MAX":
        ...
        draw_circle_on_map(toronto_coords[0], toronto_coords[1], max(diameters),names[index]) #The biggest asteroid name and size

    elif int(display_choice) <= len(diameters):
         index = int(display_choice)-1
         draw_circle_on_map(toronto_coords[0], toronto_coords[1], diameters[index],names[index]) #The # asteroid and  size
